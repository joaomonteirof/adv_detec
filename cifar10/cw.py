...
class AttackCarliniWagnerL2:
	...
	def __init__(self, targeted=True, num_classes=10, search_steps=None, max_steps=None, cuda=True, debug=False):
		self.debug = debug
	...
	def run(self, model, input, target, batch_idx=0):
		...
		return Variable(torch.from_numpy(o_best_attack).contiguous().view(o_best_attack.shape[0], o_best_attack.shape[2], o_best_attack.shape[1], -1))
