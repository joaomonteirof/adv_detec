...
for i in (range(len(attacks))):
	for j in (range(len(attacks))):
		...
		if i != j:
			...
			traindata_file = open('detec_'+attacks[i]+'.p', 'rb')
			...
			testdata_file = open('detec_'+attacks[j]+'.p', 'rb')
...
np.savetxt('cifar10_rfo.csv', results_rfo, delimiter=',')
np.savetxt('cifar10_knn.csv', results_knn, delimiter=',')
np.savetxt('cifar10_svm.csv', results_svm, delimiter=',')
