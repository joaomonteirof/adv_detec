...
for i in range(ndata):
	...
	if np.random.rand() > 0.5:
		...
		if np.random.rand() > 0.5:
			attack_sample = fgsm(model=model_1, x=clean_sample, y=target, eps=eps_)

		else:
			attack_sample = fgsm(model=model_2, x=clean_sample, y=target, eps=eps_)


		sample = np.concatenate([attack_sample.view(1,-1).cpu().numpy(), np.ones([1,1])], 1)

	else:

		sample = np.concatenate([clean_sample.view(1,-1).cpu().numpy(), np.zeros([1,1])], 1)

	data.append(sample)

data = np.asarray(data)

pfile = open('./data_viz.p', 'wb')
