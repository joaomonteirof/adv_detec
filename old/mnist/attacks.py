...
def deepfool(model, x, num_classes=None, overshoot=0.02, max_iter=50):
	...
	f_image = model.forward(x0, pre_softmax = True)
	...
	x = Variable(pert_image.data, requires_grad=True)
	fs = model(x, pre_softmax = True)
	k_i = label
	...
	while k_i == label and loop_i < max_iter:
		...
		for k in range(1, num_classes):
			...
			print(I[0])
			print(I[k])
			print(cur_grad.norm(2))
			print(grad_orig.norm(2))
		...
		x.data = pert_image.data
		fs = model.forward(x, pre_softmax = True)
